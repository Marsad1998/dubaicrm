...
//         {/* ── Contacts Sidebar ── */}
//         <div
//           className={`flex-none w-[320px] absolute xl:relative z-10 xl:h-full hidden xl:flex flex-col overflow-hidden rounded-md shadow ${
//             isShowChatMenu ? '!flex' : ''
//           }`}
...
//             </div>
//             <button type="button" className="xl:hidden text-[#54656f]" onClick={() => setIsShowChatMenu(false)}>
//               <IconMenu />
//             </button>
//           </div>
...
//                   style={{
//                     borderColor: '#e9edef',
//                     background: selectedContact?.phone === contact.phone ? '#ffffff' : 'transparent',
//                   }}
...
//                 <button
//                   className="xl:hidden mt-5 px-6 py-2 rounded-full text-white text-sm font-medium bg-[#25d366]"
...
//                   <div className="flex items-center space-x-3">
//                     <button type="button" className="xl:hidden text-[#54656f]" onClick={() => setIsShowChatMenu(true)}>
//                       <IconMenu />
//                     </button>
...
//                     {/* <button type="button" className="p-2 rounded-full hover:bg-[#d9dbe0] transition-colors"><IconPhone /></button> */}
//                     {/* <button type="button" className="p-2 rounded-full hover:bg-[#d9dbe0] transition-colors"><IconVideo /></button> */}
//                     <button type="button" className="p-2 rounded-full hover:bg-[#d9dbe0] transition-colors" onClick={() => setShowProfile((v) => !v)}><IconHorizontalDots /></button>
//                   </div>
//                 </div>
...
//                           new Date(message.message_time).toDateString() !== new Date(messages[index - 1]?.message_time).toDateString();
...
//                             style={{
//                               color: profileTab === tab ? '#25d366' : '#667781',
//                               borderBottom: profileTab === tab ? '2px solid #25d366' : '2px solid transparent',
//                             }}
//                           >
//                             {tab}
//                           </button>
...
//                                   <svg className="w-4 h-4" fill="#34b7f1" viewBox="0 0 24 24">
...
const Chat = () => {
  ...
  // Setup WebSocket listener - FIXED VERSION
  useEffect(() => {
    const channel = echo.channel('whatsapp-messages');
    ...
    channel.listen('.message.sent', (e: { chat: IncomingMessageData }) => {
      ...
      if (chat.direction === 'inbound') {
        // For inbound messages, update contacts list
        updateContactsFromMessage(phone, { 
          body: messageBody, 
          created_at: chat.created_at, 
          status: chat.status 
  ...
  }, [selectedContact]); // Removed updateContactsFromMessage from dependencies if not needed
  ...
  const updateContactsFromMessage = (phone: string, messageData: { body: string; created_at: string; status: string }) => {
    setContacts(prev => {
      ...
      if (existingContact) {
        ...
        // Add new contact (only for inbound messages)
        const newContact: ChatContact = {
          phone,
          last_message: messageData.body,
          last_message_time: messageData.created_at,
