...
import Toast from '../../services/toast';
import { setPageTitle } from '../../slices/themeConfigSlice';
import { destoryLeads, assignleads, reassigleads, allLeads } from '../../slices/leadsSlice';
import Select from 'react-select';
import LeadModal from '../../components/LeadModal';
import '../dashboard/dashboard.css'; 
import Swal from 'sweetalert2';
...
const ReAssign = () => {
    ...
    const AssignLead = async (agentId: number, phone: number) => {
        ...
        try {
            ...
            if (response.payload.status === 200 || response.payload.status === 201){
                ...
                dispatch(reassigleads({
                    page: 1, // Go to first page after assignment
                    perPage: per_page,
                    sortField: sortStatus.columnAccessor,
                    sortOrder: sortStatus.direction,
                    search: searchTerm
        ...
        if (result.isConfirmed) {
            try {
                ...
                if (response.payload.status === 200 || response.payload.status === 201) {
                    ...
                    dispatch(reassigleads({
                        page: 1, // Go to first page after deletion
                        perPage: per_page,
                        sortField: sortStatus.columnAccessor,
                        sortOrder: sortStatus.direction,
                        search: searchTerm
    ...
    const handlePageChange = (page: number) => {
        // Direct dispatch without waiting for state update
        dispatch(reassigleads({ 
            page: page,
            perPage: per_page,
            sortField: sortStatus.columnAccessor,
    ...
    const onSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Direct dispatch when search changes
        dispatch(reassigleads({ 
            page: 1,
            perPage: per_page,
            sortField: sortStatus.columnAccessor,
                ...
                return (
                    <button 
                        ...
                        onClick={() => { 
                            ...
                            setSelectedLead((fullLead as any)?.comments || []); 
                        }} 
                    > 
                        View 
                    </button>
                ...
                <div className="flex items-center space-x-2">
                    <div className="w-full md:w-[200px]">
                        <Select 
                            ...
                            options={transformedAgents} 
                            isDisabled={SearchagentSelected} 
                            classNamePrefix="custom-select" 
                            className="custom-multiselect z-10" 
                            value={transformedAgents.find(option => option.value === selectedAgent) || null}
                            onChange={(selectedOption) => { 
