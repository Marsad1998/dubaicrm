...
import { newleads, destoryLeads, assignleads, pollLeads, allLeads } from '../../slices/leadsSlice';
...
const ReAssign = () => {
    ...
    const handleSelectAllPages = async (isChecked: boolean) => {
        if (isChecked) {
            ...
            try {
                setLoading(true);
                const response = await dispatch(pollLeads({
                    page: 1,
                    perPage: total,
                    sortField: sortStatus.columnAccessor,
                    sortOrder: sortStatus.direction,
    ...
    const columns = [
        ...
        { 
            ...
            sortable: true,
            render: (record: any) => {
                return <span className="badge bg-success">Poll Leads</span>;
            },
        },
        ...
        <div>
            <div className="panel flex items-center justify-between overflow-visible whitespace-nowrap p-3 text-dark relative">
                <div className="flex items-center">
                    ...
                    <span className="ltr:mr-3 rtl:ml-3">Details of Your Poll Leads: </span>
                </div> 
